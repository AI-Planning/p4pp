...
:effect (and (not (empty ?s)) (full ?s ?x) (have_soil_analysis ?x ?p) (not (at_soil_sample ?p))
		)
)
...
:effect (and (not (empty ?s)) (full ?s ?x) (have_rock_analysis ?x ?p) (not (at_rock_sample ?p))
		)
)

(:action drop
:parameters (?x - rover ?y - store)
:precondition (and (store_of ?y ?x) (full ?y ?x)
		)
:effect (and (not (full ?y ?x)) (empty ?y)
	)
)
